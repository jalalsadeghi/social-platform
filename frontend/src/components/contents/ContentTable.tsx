...
interface Content {
  ...
  status: string;
  created_at: string;
  platforms_status: PlatformStatus[];
}

const fetchContents = async ({ pageParam = 0 }): Promise<Content[]> => {
  const res = await api.get(`/contents?skip=${pageParam}&limit=30`);
  return res.data;
};
...
export const ContentTable = () => {
  ...
  const contentsQuery = useQuery({
    queryKey: ["contents"],
    queryFn: () => fetchContents({ pageParam: 0 }),
  });
                ...
                <TableRow key={content.id}>
                  ...
                  <TableCell>
                    <img
                      ...
                      onClick={() =>
                        window.open(`${baseURL}/${content.thumb_filename}`, "_blank")
                      }
                    />
                  </TableCell>
